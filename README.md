...
* [`debian` Dockerfile](https://github.com/mkenney/docker-npm/blob/master/Dockerfile)
  ...
  ![Image size](https://img.shields.io/badge/image size-~300MB-blue.svg) This image is under development and may not be as stable as versioned images. This image is based on [`node:latest`](https://hub.docker.com/r/library/node/tags/latest/). Package versions are not pinned, instead the included `npm` executable is used to install current versions of the packages.
